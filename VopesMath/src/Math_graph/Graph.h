...
private:
	...
	void DFS(int v, bool visited[]); // 深度优先搜索：子过程
};
...
void Graphlnk<T, E>::DFS(const T& v) {
	int i, loc, n = numVertices; // 取图中的顶点个数
	bool *visited = new bool[n]; // 创建辅助数组

	for (i = 0; i < n; i++) // 初始化未被访问过
		visited[i] = false;
	loc = getVertexPos(v); // 获取顶点的位置
	DFS(loc, visited); // 从顶点开始深度优先搜索
	cout << endl;
	delete[]visited; // 释放空间
}
...
void Graphlnk<T, E>::DFS(int v, bool visited[]) {
	...
	w = getFirstNeighbor(v); // 找顶点v的第一个邻接顶点w
	while (w != -1) {
		if (visited[w] == false)
			DFS(w, visited); // 若w未访问过，递归访问顶点w
		w = getNextNeighbor(v, w); // 取v顶点的邻接顶点w的下一个邻接顶点
	}
}
...
void Graphlnk<T, E>::BFS(const T& v) {
	int i, w, loc, n = numVertices; // 取图中的顶点个数
	queue<int> q; // 队列
	bool *visited = new bool[n]; // 创建辅助数组

	for (i = 0; i < n; i++) // 初始化未被访问过
		visited[i] = false;
	loc = getVertexPos(v); // 获取第一个顶点号
	cout << getValue(loc) << " "; // 或写成：cout << v << " "; 访问顶点v
	visited[loc] = true; // 作已访问标记
	q.push(loc); // 进队列，实现分层访问
	while (!q.empty()) { // 队列不为空
		loc = q.front(); // 取队头元素
		q.pop(); // 退出队列
		w = getFirstNeighbor(loc); // 找顶点loc的第一个邻接顶点w
		while (w != -1) { // 若邻接顶点w存在
			if (visited[w] == false) { // 若未访问过
				cout << getValue(w) << " "; // 访问顶点w
				visited[w] = true; // 作访问过
				q.push(w); // 顶点w进队列
			}
			w = getNextNeighbor(loc, w); // 找顶点loc的下一个邻接顶点
		}
	} // while结束
	cout << endl;
	delete[]visited; // 释放空间
